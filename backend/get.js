...
//::WindowPath->[File]
get_bush_ffns = function(path) {//ffns = full filenames
  var flds, files;
  flds = folders(path);
  files = full_filenames(path);
  if (flds.length) {
    return files.concat(flds.map(fld=>get_bush_ffns(fld))).reduce((a,b)=>{return a.concat(b);},[]);
  } else {
    return files;
  }
};
...
exports.diary = (x) => {
  ...
  if (x.year !== undefined) {
    ...
    sql += ' WHERE objects.creation_timestamp >=' + from / 1000 + ' AND \
		objects.creation_timestamp <=' + to / 1000;
  }
...
directory_filenames = function(path) {//::String->[String]
  var all, res, pared,stats, fullname, timestamp;
  all = fs.readdirSync(path);
  res = all.map((filename)=>{
    fullname = path + '\\' + filename;
...
exports.last_edited = function(path) {
  ...
  vh = filenames.map((filename)=>{return [fs.statSync(path + '\\' + filename)['mtime'].getTime(), filename];});
  vh.sort((a, b)=>{return b[0] - a[0];});
  return vh[0][1];
};
